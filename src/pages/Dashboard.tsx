...
export const Dashboard = () => {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 md:p-8 overflow-auto max-w-[1600px] mx-auto w-full">
      {/* Header */}
      <div className="mb-2">
      ...
      >
        <div className="bg-card border border-border rounded-2xl shadow-sm p-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            ].map(({ icon: Icon, color, bg, msg, time }) => (
              <div key={msg} className="flex gap-3 group cursor-pointer">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-2xl shadow-sm p-0 overflow-hidden flex flex-col">
          ...
          <div className="overflow-x-auto flex-1 p-2">
            <table className="w-full text-sm">
              <thead>
                <tr>
                  {["Application ID", "Applicant Name", "Loan Type", "Analyst", "Risk", "Status", "Time"].map(h => (
                    <th key={h} className="text-left py-3 px-4 text-muted-foreground font-semibold text-xs uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {activity.map(row => (
                  <tr key={row.doc} className="border-b border-border/50 hover:bg-secondary/50 transition-colors group">
                    <td className="py-3 px-4 font-mono font-medium text-blue-600 cursor-pointer hover:underline"
                      onClick={() => navigate("/viewer")}>{row.doc}</td>
