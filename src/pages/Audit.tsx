...
export const Audit = () => {
  ...
  const filtered = auditLogs.filter(
    l => l.doc.toLowerCase().includes(search.toLowerCase()) ||
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="text-sm border border-border rounded-xl pl-9 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500/50 bg-background w-full sm:w-64 transition-shadow"
                />
              </div>
            ...
            <thead>
              <tr>
                {["Log ID", "Timestamp", "User", "Action", "Document", "Status"].map(h => (
                  <th key={h} className="text-left py-4 px-4 text-muted-foreground font-semibold uppercase tracking-wider text-xs">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
          ...
          {auditLogs.slice(0, 5).map((log, i) => (
            <div key={log.id} className="flex gap-5">
              <div className="flex flex-col items-center">
                <div className={`w-3 h-3 rounded-full shrink-0 mt-1 shadow-sm ring-4 ring-white ${
