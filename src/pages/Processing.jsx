...
export const Processing = () => {
    ...
    return (<div className="p-6 md:p-8 flex flex-col gap-6 overflow-auto max-w-3xl mx-auto w-full">
      <div className="mb-2">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="bg-card border border-border rounded-2xl shadow-sm p-8">
        ...
        <div className="mb-10">
          ...
          <div className="h-2.5 bg-secondary rounded-full overflow-hidden shadow-inner">
            <motion.div initial={{ width: 0 }} animate={{ width: "50%" }} transition={{ duration: 1, delay: 0.2 }} className="h-full bg-blue-600 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 relative">
        ...
        <div className="flex flex-col">
          {steps.map((step, i) => (<motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} key={step.id} className="flex gap-5">
              ...
              <div className="flex flex-col items-center">
                ...
                {i < steps.length - 1 && (<div className={`w-0.5 flex-1 my-1.5 rounded-full ${step.status === "done" ? "bg-emerald-500" : "bg-border"}`}/>)}
              </div>
              {/* Content */}
              <div className={`pb-8 flex-1 ${i === steps.length - 1 ? "pb-0" : ""}`}>
                ...
                {step.status === "active" && (<div className="mt-3 h-2 bg-secondary rounded-full overflow-hidden w-64 max-w-full">
        ...
        <div className="mt-6 pt-6 border-t border-border flex flex-col sm:flex-row gap-3">
          <button onClick={() => navigate("/analysis")} className="bg-blue-600 text-white text-sm font-semibold px-6 py-2.5 rounded-xl hover:bg-blue-700 hover:shadow-lg transition-all flex items-center justify-center gap-2 sm:flex-1">
      ...
      <div className="bg-card border border-border rounded shadow-sm p-5">
        ...
        ].map(([label, value]) => (<div key={label} className="bg-secondary rounded p-2.5">
