...
import { XCircle, AlertTriangle, CheckCircle, FileCheck } from "lucide-react";
...
export const Analysis = () => {
  ...
  return (
    <div className="p-6 md:p-8 flex flex-col gap-6 overflow-auto max-w-6xl mx-auto w-full">
      <div className="mb-2">
          ...
          <div className="relative w-40 h-40 mb-6 drop-shadow-md">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              ...
              <circle
                ...
                strokeLinecap="round"
                className="animate-[stroke_1.5s_ease-out_forwards]"
              />
            </svg>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }} className="lg:col-span-2 bg-card border border-border rounded-2xl shadow-sm p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={label} className={`${bg} border border-border/50 rounded-xl p-4 shadow-sm`}>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }} className="bg-card border border-border rounded-2xl shadow-sm p-8">
        ...
        <div className="flex flex-col gap-4">
          {checks.map((check, i) => (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + (i * 0.05) }} key={check.label} className={`border rounded-xl p-5 shadow-sm transition-colors hover:shadow-md ${
              check.status === "danger" ? "border-red-200 bg-red-50"
              : check.status === "warning" ? "border-yellow-200 bg-yellow-50"
              : "border-emerald-200 bg-emerald-50"
            }`}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
                <div className="flex items-center gap-3">
                  {check.status === "danger" ? <XCircle size={18} className="text-red-600 shrink-0" />
                    : check.status === "warning" ? <AlertTriangle size={18} className="text-yellow-600 shrink-0" />
        ...
        <div className="mt-8 pt-6 border-t border-border flex flex-col sm:flex-row gap-4">
          ...
          >
            Generate Detailed Report <FileCheck size={16} />
          </button>
          <button
            onClick={() => navigate("/viewer")}
            className="border border-border text-foreground font-medium text-sm px-6 py-2.5 rounded-xl hover:bg-secondary transition-colors sm:w-auto"
          >
