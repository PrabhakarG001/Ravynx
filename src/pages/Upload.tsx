...
import { Upload as UploadIcon, Cpu, FileText, X, CheckCircle } from "lucide-react";
import { Badge } from "../components/common/Badge";

export const Upload = () => {
  const navigate = useNavigate();
  const [files, setFiles] = useState<{ name: string; size: number; type: string; status: string }[]>([]);
  ...
  return (
    <div className="p-6 md:p-8 flex flex-col gap-6 overflow-auto max-w-4xl mx-auto w-full">
      <div className="mb-2">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="bg-card border border-border rounded-2xl shadow-sm p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 border-b border-border pb-8">
          <div>
            ...
            <input
              ...
              className="w-full border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 bg-background transition-all"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <select
              ...
              className="border border-border rounded-xl text-sm px-4 py-2.5 bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/50 w-full cursor-pointer transition-shadow"
            >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <select
            ...
            className="border border-border rounded-xl text-sm px-4 py-2.5 bg-background focus:outline-none focus:ring-2 focus:ring-blue-500/50 w-full sm:w-64 cursor-pointer transition-shadow"
          >
        ...
        <div
          className="border-2 border-dashed border-border rounded-2xl bg-secondary/30 p-12 text-center cursor-pointer hover:border-blue-500/50 hover:bg-blue-50/50 transition-all group"
      ...
      <AnimatePresence>
        {files.length > 0 && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-card border border-border rounded-2xl shadow-sm p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
              <h3 className="text-base font-semibold text-foreground">Files Queued ({files.length})</h3>
              <button
                onClick={() => navigate("/processing")}
                className="bg-blue-600 text-white text-sm font-semibold px-6 py-2.5 rounded-xl hover:bg-blue-700 hover:shadow-lg transition-all flex items-center justify-center gap-2"
              >
                <Cpu size={16} /> Submit Application
              </button>
            </div>
            <div className="flex flex-col gap-3">
              {files.map((f, i) => (
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} key={i} className="flex items-center gap-4 border border-border rounded-xl p-3 bg-secondary/10 group">
      ...
      {files.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="bg-card border border-border rounded-2xl shadow-sm p-6 mt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={type} className="flex gap-3 bg-secondary/30 p-3 rounded-xl border border-border/50">
