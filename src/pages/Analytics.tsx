...
import { AlertTriangle, TrendingUp, Cpu, Users } from "lucide-react";
import { StatCard } from "../components/cards/StatCard";
import { monthlyData, processingTimeData, riskTypeData, COLORS } from "../utils/mockData";

export const Analytics = () => {
  return (
    <div className="p-6 md:p-8 flex flex-col gap-6 overflow-auto max-w-[1600px] mx-auto w-full">
      <div className="mb-2">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="overflow-x-auto flex-1 p-2">
            <table className="w-full text-sm">
              <thead>
                <tr>
                  {["Analyst Name", "Docs Processed", "Flagged", "Cleared", "Avg Review Time (min)"].map(h => (
                    <th key={h} className="text-left py-3 px-4 text-muted-foreground font-semibold uppercase tracking-wider text-xs">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={name as string} className="border-b border-border/50 hover:bg-secondary/50 transition-colors">
                    <td className="py-4 px-4 font-bold text-foreground flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs">{(name as string).split(' ').map(n=>n[0]).join('')}</div>
                      {name}
                    </td>
