...
export default function Landing() {
    ...
    const team = [
        ...
        { name: "Sakhsam Varshaney", role: "Backend Developer", initial: "SV" },
    ];
    const navLinks = [
  ...
  const cardNavItems = [
    ...
    return (<div className="min-h-screen bg-background font-['Plus_Jakarta_Sans',sans-serif]">
        ...
        >
          ...
          <div className="flex items-center lg:gap-14 gap-8">
            <div className="cursor-pointer" onClick={() => navigate("/")}>
              <span className={`text-[28px] font-['Inter',sans-serif] font-semibold tracking-[-0.5px] transition-colors ${isLightNavbar ? 'text-foreground' : 'text-white'}`}>Ravynx</span>
            </div>
            
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map(link => (
                <a key={link.name} href={link.href} className={`group relative font-semibold text-[15px] transition-all duration-300 ${isLightNavbar ? 'text-foreground' : 'text-white'}`}>
                  <span className="relative inline-block transition-transform duration-300 group-hover:-translate-y-[2px] group-hover:scale-105">
                    {link.name}
                  </span>
                  <span className={`absolute -bottom-1 left-0 h-[2px] w-0 group-hover:w-full transition-all duration-300 ease-out rounded-full ${isLightNavbar ? 'bg-primary' : 'bg-white'}`} />
                </a>
              ))}
            </div>
          </div>
      ...
      <div className={`md:hidden fixed top-0 left-0 w-full z-50 transition-transform duration-300 ${navVisible ? 'translate-y-0' : '-translate-y-full'}`}>
        <CardNav logo={<span className={`text-[28px] font-['Inter',sans-serif] font-semibold tracking-[-0.5px] transition-colors ${isLightNavbar ? 'text-black' : 'text-white'}`}>Ravynx</span>} items={cardNavItems} baseColor={isLightNavbar ? "rgba(255, 255, 255, 0.9)" : scrolled ? "rgba(0, 0, 0, 0.3)" : "transparent"} menuColor={isLightNavbar ? "#000000" : "#ffffff"} buttonBgColor="#2563eb" buttonTextColor="#ffffff" onLoginClick={() => navigate("/login")} onSignupClick={() => navigate("/login")} scrolled={scrolled} isLightNavbar={isLightNavbar} />
      </div>
      ...
      <div ref={containerRef} id="capabilities" className="relative h-[200vh] bg-black">
        <div className="sticky top-0 h-screen overflow-hidden flex flex-col justify-between">
          ...
          <motion.div className="absolute inset-0 z-0 h-full w-full flex flex-col items-center justify-start pt-32 overflow-hidden" style={{ opacity: heroOpacity }}>
             ...
             <div className="relative z-20 w-full max-w-4xl mx-auto px-6 flex flex-col items-center text-center mt-4">
               <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }} className="text-4xl sm:text-5xl md:text-6xl lg:text-[64px] font-bold tracking-tight leading-[1.1] mb-6">
               ...
               <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="text-white/90 text-[16px] sm:text-[18px] md:text-[20px] leading-[1.6] max-w-3xl mx-auto mb-4 font-medium">
               ...
               <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.25 }} className="text-white/60 text-[14px] sm:text-[15px] md:text-[16px] leading-[1.6] max-w-2xl mx-auto mb-10">
               ...
               <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="flex flex-col sm:flex-row items-center gap-6 relative z-50 pointer-events-auto">
                 <button onClick={() => navigate("/login")} className="px-8 py-2.5 rounded-full border border-purple-500 text-white text-sm font-medium hover:bg-purple-500/10 transition-colors shadow-[0_0_15px_rgba(168,85,247,0.3)] inline-flex items-center justify-center gap-2 cursor-pointer">
                   Launch Dashboard <ArrowRight className="w-4 h-4" strokeWidth={2.5} />
                 </button>
                 <button className="px-8 py-2.5 rounded-full border border-white/30 text-white text-sm font-medium hover:bg-white/10 transition-colors inline-flex items-center justify-center gap-2 cursor-pointer">
             ...
             {/* 3D Illustration */}
             <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.4 }} className="relative w-full max-w-5xl mx-auto mt-4 flex-1 flex items-end justify-center pointer-events-none">
               <img src="/serendale_hero_bg.jpg" alt="3D Infinity Robot" className="w-full h-auto object-contain relative z-10" style={{ maskImage: 'linear-gradient(to bottom, black 60%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, black 60%, transparent 100%)' }} />
      ...
      <section id="how-it-works" className="bg-secondary/50 py-24 border-y border-border overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-2 md:gap-0 relative">
            ...
            { icon: FiGitMerge, title: "Risk Engine", desc: "Cross-validate facts" },
      ...
      <section className="px-6 md:px-12 py-24 max-w-4xl mx-auto">
        ...
        <div className="bg-[#0f172a] border border-white/10 rounded-xl overflow-hidden shadow-2xl font-mono relative">
          <div className="flex items-center px-4 py-3 border-b border-white/10 bg-black/40">
            ...
            <div className="ml-4 text-xs text-slate-400">ravynx-node-process</div>
          </div>
          <div className="p-6 text-sm text-gray-300 h-72 overflow-y-auto">
            ...
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false, margin: "-50px" }} transition={{ staggerChildren: 0.8 }} className="space-y-4">
              ...
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-3 text-red-400 font-semibold bg-red-500/10 p-2 rounded border border-red-500/20">
                <span className="shrink-0">[ALERT]</span> <span>Metadata modified date (2026-08-05) does not match creation date. Possible tampering detected.</span>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-3">
                <span className="text-blue-400 shrink-0">[INFO]</span> <span>Cross-validating PAN & GST records via Gov API...</span>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-3 text-red-400 font-semibold bg-red-500/10 p-2 rounded border border-red-500/20">
                <span className="shrink-0">[ALERT]</span> <span>Owner name mismatch detected: "Rahul Sharma" (Doc) vs "Rahul Kumar Sharma" (Registry)</span>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-3">
      ...
      <section className="relative py-32 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false, margin: "-50px" }} transition={{ duration: 0.6 }} className="max-w-4xl mx-auto text-center mt-12 bg-black/40 backdrop-blur-sm border border-white/10 rounded-2xl p-10">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 md:mb-24">
            <div className="max-w-2xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-semibold tracking-wider text-blue-400 uppercase">Enterprise Grade</span>
              </div>
          ...
          <div className="relative rounded-[2.5rem] overflow-hidden bg-[#030213] border border-white/10 shadow-2xl">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: false, margin: "-50px" }} transition={{ duration: 0.8 }} className="relative z-10 text-center text-white py-20 px-6">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <button onClick={() => navigate("/login")} className="w-auto bg-white text-black font-bold text-sm px-6 py-3 rounded-full hover:bg-gray-100 transition-all shadow-lg hover:scale-105 inline-flex items-center justify-center gap-2">
                  Launch Dashboard <ArrowRight className="w-4 h-4" strokeWidth={2.5} />
                </button>
                <button className="w-auto bg-white/10 border border-white/20 backdrop-blur-md text-white font-bold text-sm px-6 py-3 rounded-full hover:bg-white/20 transition-all inline-flex items-center justify-center gap-2">
                  <PlayCircle className="w-4 h-4" strokeWidth={2.5} /> Try Demo
                </button>
              </div>
            </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-8 mb-16">
            <div className="col-span-2 lg:col-span-2 flex flex-col items-center sm:items-start text-center sm:text-left">
              <span className="text-[28px] font-['Inter',sans-serif] font-bold tracking-[-0.5px] text-white mb-4 block">Ravynx</span>
              <p className="text-white/60 text-sm leading-relaxed mb-6 max-w-xs sm:max-w-none">
        ...
        {activeModal && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-6" onClick={() => setActiveModal(null)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-card border border-border max-w-lg w-full rounded-2xl p-8 relative shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              {activeModal.category === 'Platform' && (<p className="text-foreground leading-relaxed mb-8">
                  You're exploring our <strong>{activeModal.title}</strong> features! As this project is a hackathon MVP built by <strong>Prabhakar Gupta</strong> and <strong>Saksham Varshaney</strong>, we are currently focused on demonstrating our core AI processing engine. Check out the "How it Works" section to see the platform in action!
                </p>)}
              
              {activeModal.category === 'Company' && (<p className="text-foreground leading-relaxed mb-8">
                  Interested in our <strong>{activeModal.title}</strong>? Ravynx is an ambitious hackathon project created by developers <strong>Prabhakar Gupta</strong> and <strong>Saksham Varshaney</strong>. We're currently a two-person powerhouse building the future of financial underwriting. Reach out to us directly via our LinkedIn profiles in the Team section if you want to connect!
                </p>)}
              
              {activeModal.category === 'Legal' && (<p className="text-foreground leading-relaxed mb-8">
                  Ah, the fine print for <strong>{activeModal.title}</strong>! Since Ravynx is a 48-hour hackathon prototype built by <strong>Prabhakar Gupta</strong> and <strong>Saksham Varshaney</strong>, our official legal team (which is also just the two of us) hasn't drafted these documents yet. Rest assured, our AI is designed with bank-grade security and compliance in mind!
                </p>)}
