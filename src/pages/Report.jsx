...
import { HiBuildingOffice2, HiArrowDownTray, HiArrowPath, HiDocumentText, HiCpuChip, HiXCircle, HiExclamationTriangle, HiCheckCircle } from "react-icons/hi2";
export const Report = () => (<div className="p-6 md:p-8 flex flex-col gap-6 overflow-auto max-w-5xl mx-auto w-full">
    {/* Header */}
      ...
      <div className="flex items-start justify-between flex-wrap gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center">
              <HiBuildingOffice2 size={16} className="text-white"/>
            </div>
        ...
        <div className="flex gap-3">
          <button className="bg-blue-600 text-white text-sm font-bold px-5 py-2.5 rounded-xl hover:bg-blue-700 hover:shadow-lg transition-all flex items-center gap-2">
            <HiArrowDownTray size={16}/> Download PDF
          </button>
          <button className="border-2 border-border text-foreground font-bold text-sm px-5 py-2.5 rounded-xl hover:bg-secondary flex items-center gap-2 transition-colors">
            <HiArrowPath size={16}/> Regenerate
          </button>
        </div>
      </div>
    </motion.div>
    ...
    ].map(([label, val, textCls, bgCls, borderCls]) => (<div key={label} className={`bg-card border ${borderCls} ${bgCls} rounded-2xl p-6 shadow-sm text-center transition-transform hover:-translate-y-1`}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.2 }} className="bg-card border border-border rounded-2xl shadow-sm p-6 md:p-8">
        <h3 className="text-base font-bold text-foreground mb-4 border-b border-border pb-3 flex items-center gap-2"><HiDocumentText size={18} className="text-blue-600"/> Application Information</h3>
    ...
    ].map(([label, val, mono]) => (<div key={label} className="flex justify-between items-center border-b border-border/50 pb-2 last:border-0 last:pb-0">
      ...
      <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.2 }} className="bg-card border border-border rounded-2xl shadow-sm p-6 md:p-8 flex flex-col justify-between">
        <div>
          <h3 className="text-base font-bold text-foreground mb-4 border-b border-border pb-3 flex items-center gap-2"><HiCpuChip size={18} className="text-purple-600"/> AI Insights & Recommendation</h3>
          <div className="bg-red-50 border border-red-200 rounded-xl p-5 mb-5 shadow-sm">
            <p className="text-sm text-red-800 font-black mb-3 flex items-center gap-2"><HiXCircle size={18}/> REJECT — Do Not Process Loan</p>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.3 }} className="bg-card border border-border rounded-2xl shadow-sm p-6 md:p-8 mb-8">
      <h3 className="text-base font-bold text-foreground mb-6 border-b border-border pb-3 flex items-center gap-2"><HiExclamationTriangle size={18} className="text-amber-500"/> Key Findings</h3>
    ...
    ].map((item, i) => (<div key={item.title} className={`flex gap-4 rounded-xl p-5 border shadow-sm transition-transform hover:-translate-y-1 ${item.sev === "danger" ? "bg-red-50 border-red-200"
            : item.sev === "warning" ? "bg-amber-50 border-amber-200"
                : "bg-emerald-50 border-emerald-200"}`}>
            {item.sev === "danger" ? <HiXCircle size={20} className="text-red-600 shrink-0 mt-0.5"/>
            : item.sev === "warning" ? <HiExclamationTriangle size={20} className="text-amber-600 shrink-0 mt-0.5"/>
                : <HiCheckCircle size={20} className="text-emerald-600 shrink-0 mt-0.5"/>}
            <div>
